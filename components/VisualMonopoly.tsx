...
const VisualMonopoly: React.FC = () => {
  ...
  return (
    <section className="h-full w-full bg-white relative overflow-hidden flex flex-col justify-center py-12 md:py-24">
      <div className="container mx-auto px-6 h-full flex flex-col">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 md:gap-16 items-start mb-10 md:mb-20 shrink-0">
          <div className="lg:col-span-8">
            <div className="flex items-center gap-4 md:gap-5 mb-6 md:mb-10">
               <div className="w-8 md:w-12 h-[1px] bg-gold-500"></div>
        ...
        <div className="relative w-full flex-1 rounded-[2.5rem] md:rounded-[4rem] overflow-hidden border border-slate-200 shadow-3xl bg-slate-50 group min-h-[400px]">
          <img 
            src="https://renderatelier.com/wp-content/uploads/2023/05/1-7-1-1-scaled.jpg" 
            className={`w-full h-full object-cover transition-all duration-[3s] ${activePoint ? 'scale-105 brightness-[0.35] blur-[1px]' : 'brightness-105'}`}
          ...
          <div className="absolute top-6 left-6 md:top-12 md:left-12 flex items-center gap-4 md:gap-6 z-30 pointer-events-none">
             <div className="w-3 h-3 md:w-4 md:h-4 bg-gold-500 rounded-full animate-pulse shadow-[0_0_20px_#D4AF37]"></div>
             <div className="space-y-0.5 md:space-y-1">
                ...
                <div className="text-gold-500/60 font-mono text-[6px] md:text-[8px] uppercase tracking-widest">Structural_Mesh_v2.5_Synced</div>
             </div>
          </div>
          ...
          {points.map((p) => (
            ...
            >
              <div className={`relative flex items-center justify-center cursor-pointer`}>
                 <div className={`absolute inset-0 rounded-full border-2 border-gold-500/50 transition-all duration-1000 ${activePoint === p.id ? 'scale-[3] md:scale-[4] opacity-0' : 'scale-100 opacity-100 animate-pulse'}`}></div>
                 <div className={`w-6 h-6 md:w-8 md:h-8 rounded-full border-2 transition-all duration-500 flex items-center justify-center backdrop-blur-xl ${activePoint === p.id ? 'bg-gold-500 border-white scale-125 md:scale-150 shadow-gold-glow' : 'bg-white/40 border-gold-500 hover:bg-white/80'}`}>
                    <div className={`w-1.5 h-1.5 md:w-2 md:h-2 rounded-full ${activePoint === p.id ? 'bg-white' : 'bg-gold-600'}`}></div>
                 </div>

                 {/* Label Tooltip - Desktop only or persistent on toggle */}
                 <div className={`absolute top-10 md:top-12 left-1/2 -translate-x-1/2 transition-all duration-500 whitespace-nowrap hidden sm:block ${activePoint === p.id ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0 group-hover:opacity-100'}`}>
          ...
          {/* Cinematic Insight Overlay - Optimized for all viewports */}
          <div className={`absolute inset-0 z-50 flex items-center justify-center p-6 md:p-12 transition-all duration-1000 pointer-events-none ${activePoint ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
             <div className="max-w-4xl w-full grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-20 items-center">
                <div className={`transition-all duration-1000 delay-100 transform text-center md:text-left ${activePoint ? 'translate-x-0 opacity-100' : '-translate-x-10 md:-translate-x-20 opacity-0'}`}>
                   <h3 className="text-white font-serif text-3xl md:text-6xl lg:text-7xl italic leading-[1.1] mb-6 md:mb-10 drop-shadow-2xl font-black">
                      {points.find(p => p.id === activePoint)?.title}
                   </h3>
                   <p className="text-slate-300 text-base md:text-2xl font-light leading-relaxed italic border-l-2 md:border-l-4 border-gold-500 pl-6 md:pl-10 max-w-md drop-shadow-lg mx-auto md:mx-0">
                ...
                <div className={`transition-all duration-1000 delay-300 transform ${activePoint ? 'translate-x-0 opacity-100' : 'translate-x-10 md:translate-x-20 opacity-0'}`}>
                   <div className="bg-white/5 backdrop-blur-3xl border border-white/10 p-8 md:p-12 rounded-[2.5rem] md:rounded-[4rem] shadow-[0_40px_80px_rgba(0,0,0,0.5)]">
                      <div className="flex justify-between items-start mb-8 md:mb-12">
                         <div className="w-12 h-12 md:w-16 md:h-16 rounded-2xl md:rounded-3xl bg-gold-500 flex items-center justify-center text-onyx-950 shadow-2xl">
                         ...
                         <div className="text-right">
                            ...
                            <div className="text-gold-500 font-mono text-base md:text-xl font-black">{points.find(p => p.id === activePoint)?.metric}</div>
                         </div>
                      </div>
                      <button className="w-full py-4 md:py-6 bg-white text-onyx-950 rounded-full text-[10px] md:text-[11px] font-black uppercase tracking-[0.4em] flex items-center justify-center gap-3 md:gap-4 hover:bg-gold-500 transition-all pointer-events-auto shadow-2xl active:scale-95">
          ...
          <div className="absolute bottom-6 left-6 right-6 md:bottom-12 md:left-12 md:right-12 z-50 flex flex-col md:flex-row justify-between items-center gap-6 md:gap-10 pointer-events-none">
             <div className="flex gap-8 md:gap-10 pointer-events-auto">
                {[
                  { l: "Monopoly Status", v: "100%", d: "Scarcity" },
                  { l: "Engineering", v: "L300", s: "BIM" }
                ].map((stat, i) => (
                  <div key={i} className="group cursor-help">
                    <div className="text-white/40 text-[7px] md:text-[8px] font-black uppercase tracking-[0.3em] md:tracking-[0.5em] mb-1 md:mb-2 group-hover:text-gold-500 transition-colors">{stat.l}</div>
                    <div className="flex items-baseline gap-1.5 md:gap-2">
                       <span className="text-white font-serif text-2xl md:text-3xl italic font-black">{stat.v}</span>
                       <span className="text-gold-600 text-[7px] md:text-[8px] font-black uppercase tracking-widest">{stat.d || stat.s}</span>
                    </div>
                  </div>
                ))}
             </div>
             ...
             <div className="flex items-center gap-4 md:gap-8 pointer-events-auto w-full md:w-auto">
                <button 
                  onClick={startTour}
                  className={`flex-1 md:flex-none group px-8 md:px-12 py-4 md:py-5 rounded-full border text-[9px] md:text-[10px] font-black uppercase tracking-[0.4em] transition-all flex items-center justify-center gap-3 md:gap-5 shadow-2xl ${isTouring ? 'bg-gold-500 text-onyx-950 border-gold-500' : 'bg-white/5 text-white border-white/20 hover:bg-white/10 hover:border-gold-500'} active:scale-95`}
                >
