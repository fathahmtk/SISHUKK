...
import { Target, ArrowRight, TrendingUp, Scan, Globe } from 'lucide-react';
...
const TheMarket: React.FC = () => {
  return (
    <div className="relative w-full h-full flex flex-col justify-center items-center bg-[#05070A] overflow-hidden group">
      ...
      {/* Background Layer: South India Temple Corridor */}
      ...
      <div className="absolute top-8 left-6 md:top-12 md:left-12 z-30 space-y-3 opacity-40">
         <div className="flex items-center gap-4 text-brass-200 font-black text-[9px] uppercase tracking-[0.5em]">
            <Globe size={14} /> Regional_Context_v1.2
         </div>
      </div>

      {/* Main Content */}
      <div className="relative z-10 text-center w-full max-w-7xl px-6 md:px-12 flex flex-col items-center">
         <span className="text-brass-400 text-[11px] md:text-[14px] font-black uppercase tracking-[1em] block mb-8 md:mb-16 animate-fade-in">
         ...
         <p className="text-slate-300 text-lg md:text-3xl font-light font-editorial italic max-w-4xl leading-relaxed mb-12 md:mb-28 opacity-80 px-4">
           Faith-driven resilience. <br className="hidden lg:block"/>
           <span className="text-white font-medium border-b border-brass-400/40 pb-3">Guruvayur serves as South India's premier spiritual anchor with 12M+ annual footfall.</span>
         </p>

         <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 md:gap-16 w-full mb-16 md:mb-32 max-w-6xl">
            {[
              { l: "Annual Visitors", v: "12 Million", s: "Inelastic Demand" },
              { l: "Daily Rituals", v: "250+", s: "Captive Wedding Hub" },
              { l: "Luxury Void", v: "Absolute", s: "No Branded Supply" }
            ].map((stat, i) => (
         ...
         <Link to="/market" className="group px-12 md:px-20 py-7 md:py-9 bg-white/5 border border-white/10 text-white rounded-full text-[11px] md:text-[12px] font-black uppercase tracking-[0.5em] hover:bg-brass-400 hover:text-onyx-950 transition-all active:scale-95 flex items-center gap-6 backdrop-blur-xl shadow-premium">
           Contextual Analysis <ArrowRight size={20} className="group-hover:translate-x-3 transition-transform duration-500" />
         </Link>
      </div>
    </div>
  );
