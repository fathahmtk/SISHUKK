  ...
  return (
    <section id="operations" className="py-48 bg-[#FDFBF7] border-t border-slate-200 relative overflow-hidden">
      <div className="container mx-auto px-6 relative z-10">
        <div className="flex flex-col lg:flex-row justify-between items-end mb-32 gap-16">
          <div className="max-w-4xl">
            <div className="flex items-center gap-5">
               ...
               <span className="text-slate-400 text-[10px] font-black uppercase tracking-[0.8em]">Chapter 06: Invisible Systems</span>
            </div>
            ...
            <p className="text-slate-500 text-2xl font-light leading-relaxed border-l-2 border-gold-400 pl-12 italic">
               "Engineered for peak-load performance. Complete segregation of resident and event visitor circulation."
            </p>
          </div>
          <div className="hidden lg:flex items-center gap-8 bg-white border border-slate-200 px-12 py-8 rounded-[3rem] shadow-2xl group transition-all hover:border-gold-400">
             <div className="w-14 h-14 rounded-2xl bg-slate-950 flex items-center justify-center text-gold-500 shadow-xl group-hover:scale-110 transition-transform">
                <Workflow size={24} />
             </div>
             <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-24">
          {[
            ...
            <div key={i} className="bg-white border border-slate-100 p-12 rounded-[4rem] flex flex-col justify-between h-[450px] hover:border-gold-400 transition-all duration-700 shadow-xl group">
               <div className="w-16 h-16 rounded-3xl bg-slate-50 flex items-center justify-center text-gold-600 group-hover:bg-gold-500 group-hover:text-white transition-all shadow-sm">
                  <item.icon size={28} />
               </div>
               <div>
                  <h3 className="text-slate-950 font-serif text-3xl mb-4 italic leading-tight">{item.title}</h3>
        ...
        <div className="text-center space-y-12 bg-white border border-slate-200 p-20 rounded-[5rem] shadow-3xl relative overflow-hidden group">
           ...
           <div className="relative z-10">
              <Link to="/operations" className="inline-flex items-center gap-10 bg-slate-950 text-white px-16 py-8 rounded-full text-[11px] font-black uppercase tracking-[0.5em] hover:bg-gold-500 transition-all shadow-3xl group active:scale-95">
                  Technical Backbone Audit <ArrowRight size={20} className="group-hover:translate-x-2 transition-transform" />
              </Link>
           </div>
        </div>
      </div>
