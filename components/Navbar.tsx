...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-500 ${isScrolled ? 'bg-luxury-black/95 backdrop-blur-md py-4 shadow-lg' : 'bg-gradient-to-b from-black/80 to-transparent py-6'}`}>
      <div className="container mx-auto px-6 flex justify-between items-center">
        ...
        <div className="flex items-center gap-3 cursor-pointer group" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
           ...
           <div className="flex flex-col">
              ...
              <span className="text-[0.6rem] text-gold-400 tracking-[0.4em] uppercase leading-none mt-1">Grand Towers</span>
           </div>
        </div>
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden absolute top-full left-0 w-full bg-luxury-black border-b border-white/10 p-6 flex flex-col space-y-6 shadow-2xl animate-fade-in-up">
          ...
          <button className="bg-gold-500 text-white w-full py-4 text-sm font-bold uppercase tracking-widest">
            Check Availability
          </button>
        </div>
      )}
