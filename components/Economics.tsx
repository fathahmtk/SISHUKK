    ...
    <div className="relative w-full h-full flex flex-col justify-center items-center bg-slate-950 overflow-hidden group">
      ...
      <div className="relative z-10 text-center w-full max-w-6xl px-6 md:px-8 flex flex-col items-center">
         ...
         <p className="text-slate-200 text-lg md:text-3xl font-light font-editorial italic max-w-4xl leading-relaxed mb-12 md:mb-24 opacity-80 px-4">
           ...
           <span className="text-white font-medium border-b border-gold-500/40 pb-2 md:pb-2 shadow-[0_4px_12px_rgba(212,175,55,0.1)]">A de-risked capital stack with 100% freehold land equity providing a massive yield hedge.</span>
         </p>
         ...
         <Link to="/economics" className="group px-12 md:px-14 py-6 md:py-7 bg-white/5 border border-white/20 text-white rounded-full text-[10px] md:text-[11px] font-black uppercase tracking-[0.4em] hover:bg-gold-500 hover:text-onyx-950 hover:border-gold-500 transition-all active:scale-95 flex items-center gap-4 md:gap-5 backdrop-blur-xl shadow-2xl">
           Economic Audit <ArrowRight size={18} className="group-hover:translate-x-2 transition-transform duration-500" />
         </Link>
      </div>

      {/* Vector HUD Grid */}
      <div className="absolute inset-0 z-20 pointer-events-none opacity-[0.03] hidden sm:block" style={{ backgroundImage: 'linear-gradient(#D4AF37 1px, transparent 1px), linear-gradient(90deg, #D4AF37 1px, transparent 1px)', backgroundSize: '60px 60px' }}></div>
    </div>
  );
};
